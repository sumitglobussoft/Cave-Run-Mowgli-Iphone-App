...
- (NSDictionary *)dictionaryFromPlistData:(NSData *)data
{
    ...
    NSDictionary *dictionaryParsed1= [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:&error];
    
    NSLog(@"dictionaryParsed1 is %@",dictionaryParsed1);
...
//        keyChainPowerBooster = [[KeychainItemWrapper alloc] initWithIdentifier:@"powerb" accessGroup:nil];
        ...
        if ([strProductId isEqualToString:@"com.globussoft.CRMnewLife"]) {
            
            int life=(int)[[NSUserDefaults standardUserDefaults] integerForKey:@"live"];
            
            NSLog(@"Purchased");
//            [[NSUserDefaults standardUserDefaults] setObject:@"5" forKey:@"live"];
             [[NSUserDefaults standardUserDefaults] setInteger:life+5 forKey:@"live"];
